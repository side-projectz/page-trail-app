...
const fetchDomainData = async (userId: string, start: string, end: string) => {
  ...
  console.log("ISO time", startTime.toISOString(), endTime.toISOString());
  console.log("local time", startTime.toUTCString(), endTime.toUTCString());

  const count = await prisma.site.findMany({
    where: {
      userId: userId,
      startDateTime: {
        gte: startTime,
  ...
  // console.log("count: " + startTime + " " + endTime, count.length);
  console.log(count[0]);
...
const processDomainData = (domains: any[]) => {
  return domains
    .map((domain: any) => ({
      ...
      timeSpent: domain.Site.reduce(
        (acc: number, site: any) =>
          acc +
          (new Date(site.endDateTime).getTime() -
            new Date(site.startDateTime).getTime()),
        0,
      ),
      formattedTimeSpent: formatTime(
        domain.Site.reduce(
          (acc: number, site: any) =>
            acc +
            (new Date(site.endDateTime).getTime() -
...
export default async function UsersDetailsDashboardCustom(
  props: UserDetailsDashboardCustomProps,
) {
  const domainsVisited = await fetchDomainData(
    props.userId,
    props.start,
    props.end,
  );
  ...
  const cardDetails = {
    ...
    totalPageVisited: processedDomains.reduce(
      (acc, domain) => acc + domain.pageCount,
      0,
    ),
    totalTimeSpent: formatTime(
      processedDomains.reduce((acc, domain) => acc + domain.timeSpent, 0),
    ),
  };
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-7">
          <Card className="col-span-4">
            ...
            <CardContent className="pl-2">
              <DashboardBarChart
                data={graphData.timeSpent.slice(0, 15)}
                xAxisKey="name"
                yAxisKey="timeSpent"
                valueKey={"timeSpent"}
              />
          ...
          <Card className="col-span-3">
            ...
            <CardContent>
              <DashboardList
                data={graphData.timeSpent.slice(0, 6)}
                titleKey="name"
                descriptionKey="LastVisitedAt"
                valueKey="formattedTimeSpent"
              />
            </CardContent>
          </Card>
        </div>
